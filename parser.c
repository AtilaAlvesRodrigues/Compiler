...
//--------------------------gerou erro(parser.c (Adicionar) – Parte 3)--------------------------
...
//-----------------------------Gerou outrp erro(parser.c (Adicionar) – Parte 6)-----------------------------
int parse_next(){
struct token* token = token_peek_next();
if (!token) return -1;
int res = 0;
...
if (node->exp.left->type != NODE_TYPE_EXPRESSION && node->exp.right && node->exp.right->type !=
...
void make_variable_node_and_register(struct history* history, struct datatype* dtype, struct token* name_token, struct node* value_node) { // LAB5 - Parte 2
